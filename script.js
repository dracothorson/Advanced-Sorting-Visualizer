...
const spirit=new Spirit(socks[0].loc,socks[1].loc,myCanvas.height*0.20);
...
function animate(){
    ...
    changed=spirit.draw(ctx)||changed;
    ...
    if(new Date().getTime()-startTime>1000 && !changed && moves.length>0){
        ...
        if(nextMove.type=="swap"){
            ...
            spirit.moveTo(socks[j].loc,socks[i].loc,false,tweenLength);
            [socks[i],socks[j]]=[socks[j],socks[i]];
        }else{ // bird is moving
            spirit.moveTo(socks[i].loc,socks[j].loc,true,tweenLength);
        }
    }
